...
+ (void)Fix {
    ...
    context[@"runError"] = ^(NSString *instanceName, NSString *selectorName) {
        NSLog(@"LYFix--runError: instanceName = %@, selectorName = %@", instanceName, selectorName);
    };
...
+ (NSString *)sandboxPathWithType:(NSInteger)type{
    if (type == 0){
        return NSTemporaryDirectory();
    }else{
        return NSSearchPathForDirectoriesInDomains(type, NSUserDomainMask, true).firstObject;
    }
}
