...
+ (BOOL)createTable:(NSString *)tableName fieldDict:(NSDictionary *)fieldDict{
    ...
    [instance.db close];
    if (isCreate) {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表创建成功！",tableName]);
    } else {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表创建失败！",tableName]);
    }
    return isCreate;
}
...
+ (BOOL)insertData:(NSString *)tableName fieldDict:(NSDictionary *)fieldDict{
    ...
    [instance.db close];
    if (isInsert) {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表插入数据成功！",tableName]);
    } else {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表插入数据失败！",tableName]);
    }
    return isInsert;
}
...
+ (BOOL)updateData:(NSString *)tableName filedDict:(NSDictionary *)fieldDict key:(NSString *)key value:(id)value{
    ...
    [instance.db close];
    if (isUpdate) {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表修改数据成功！",tableName]);
    } else {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表修改数据失败！",tableName]);
    }
    return isUpdate;
}
    ...
    if ([instance.db open]) {
        ...
        while ([set next]) {
            ...
            for (NSString *key in fieldKyes) {
                [fieldDict setObject:[set stringForColumn:key] forKey:key];
            }
...
+ (BOOL)deletedData:(NSString *)tableName key:(NSString *)key value:(id)value{
    ...
    [instance.db close];
    if (isDelete) {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表删除数据成功！",tableName]);
    } else {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表删除数据失败！",tableName]);
    }
    return isDelete;
}
...
+ (BOOL)deletedTable:(NSString *)tableName{
    ...
    if (isDelete) {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表删除表成功！",tableName]);
    } else {
        GYLog(@"%@", [NSString stringWithFormat:@"FMDB:%@表删除表失败！",tableName]);
    }
    [instance.db close];
    return isDelete;
}
